...
- **Live state**: deployed at `192.168.1.130`, reachable through `https://ha.internal`, included in PBS job `sovereign-core-nightly`.
...
3. **Configure NPM**:
   ...
   - **SSL**: assign `Sovereign Internal Wildcard`, enable Force SSL, and keep the upstream scheme `http`.
...
- **Homepage.dev (`services.yaml`)**:
  ...
  ```
- **Uptime Kuma**:
  Add an HTTP monitor targeting `https://ha.internal`. Accept `2xx/3xx` responses. Ensure the monitor runs from the internal network.
...
- **NPM 400 Bad Request Error**:
  If you access `https://ha.internal` and receive a `400 Bad Request`, it means your `trusted_proxies` configuration is missing or incorrect. Verify the IP in `configuration.yaml` includes NPM's IP and restart HA.
