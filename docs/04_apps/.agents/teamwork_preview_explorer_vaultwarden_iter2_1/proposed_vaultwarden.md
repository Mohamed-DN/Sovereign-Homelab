...
- `ADMIN_TOKEN`: Argon2 hashed token to access the `/admin` interface. (See Section 3.2 for generation instructions).
...
### 3.2 Generate Admin Token
After installing Docker, generate the Argon2 hash for the `ADMIN_TOKEN` variable:
```bash
docker run --rm -it vaultwarden/server:1.30.5 /vaultwarden hash
```
Save the resulting hash to use in your `.env` file.
...
### 3.4 Launch Container
```bash
nano .env  # Populate with the variables defined in Section 2, including the ADMIN_TOKEN
...
- `config.json` (Stores the `/admin` UI settings).
...
3. If using `.bak`, rename `db.sqlite3.bak` to `db.sqlite3`.
   - **CRITICAL**: You must also delete any existing `db.sqlite3-wal` and `db.sqlite3-shm` files to prevent database corruption.
4. Run `docker compose up -d`.
5. Access the Web Vault, verify password entries and attachments are intact.

### 5.4 Update & Rollback Procedure
Before updating the Vaultwarden container to a new version, ensure a manual backup is taken.

**To rollback an update:**
1. Stop the running container: `docker compose down`
2. Restore the previous backup of the `./vw-data` folder (and `.env` if changed). This is critical because newer versions may have migrated the database schema.
3. Edit `docker-compose.yml` to change the `image` tag back to the previous specific version (e.g., `vaultwarden/server:1.30.5`).
4. Start the container with the restored database and older image: `docker compose up -d`
