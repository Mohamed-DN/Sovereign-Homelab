...
- `ADMIN_TOKEN`: Argon2 hashed token to access the `/admin` interface. (See Section 3.1 for generation steps).
...
3. Generate the Argon2 hashed `ADMIN_TOKEN` for your `.env` file using the specific Vaultwarden version:
```bash
...
- `config.json` (Stores settings configured via the `/admin` UI).
...
3. If using `.bak`, rename `db.sqlite3.bak` to `db.sqlite3`. **Crucial**: Delete any existing `db.sqlite3-wal` and `db.sqlite3-shm` files to prevent database corruption.
4. Run `docker compose up -d`.
5. Access the Web Vault, verify password entries and attachments are intact.

### 5.4 Rollback Procedure
If an update causes issues, you must rollback both the container image and the database (since Vaultwarden migrations can be one-way).
1. Stop the container: `docker compose down`
2. Restore the SQLite database from your most recent backup (e.g., replace `db.sqlite3` with `db.sqlite3.bak`). **Crucial**: Ensure any `db.sqlite3-wal` and `db.sqlite3-shm` files are deleted.
3. Revert the `image:` tag in `docker-compose.yml` to the previously known working version (e.g., `vaultwarden/server:1.30.1`).
4. Start the container: `docker compose up -d`
5. Verify the logs for successful startup without migration errors: `docker logs -f vaultwarden`
