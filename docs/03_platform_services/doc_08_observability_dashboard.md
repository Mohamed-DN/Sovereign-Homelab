...
Live state as of 2026-06-22: 35 monitors exist in Uptime Kuma. They cover VPN, DNS, core aliases, platform aliases, deployed LXC102 apps, Immich, Nextcloud, Home Assistant, operations extensions, Forgejo SSH, Syncthing sync TCP, RustDesk TCP endpoints, Ollama API, and CrowdSec LAPI. Uptime Kuma uses SQLite in the current bootstrap deployment; the generated admin bootstrap is stored only on LXC 101 under `/root/sovereign-secrets`.
...
Deploy these only after the core dashboards are green. In the live P710 build, all three run on LXC 103:
