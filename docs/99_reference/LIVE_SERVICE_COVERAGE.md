...
| Alert email SMTP | anti-spam relay files, docs, and local self-test exist | SMTP credentials must stay local and were not committed; real delivery still needs proof | fill `/root/sovereign-secrets/alert-relay.env`, configure Kuma webhook, test DOWN/reminder/no-spam/recovery email | local secret gate |
