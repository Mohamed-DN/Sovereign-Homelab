...
| Homepage | LXC101 | `192.168.1.51` | 3002 | `dash.internal` | `http://192.168.1.51:3002` | yes | UI | YAML config + PBS | LXC101 restore drill completed | no app login by default | config/recovery notes local | Live | 29 cards, 4 read-only summary widgets, control-room theme |
...
| ntfy | LXC103 | `192.168.1.53` | 8093 | `alerts.internal` | `http://192.168.1.53:8093` | yes | `ops-ntfy` | config/cache + PBS | LXC103 restore drill completed | deny-by-default topics with separate reader/publisher/admin identities | local credential/recovery vault | Live | P0/P1 relay coverage verified |
...
| Forgejo | LXC102 | `192.168.1.52` | 3003 HTTP, 2222 SSH | `git.internal` | `http://192.168.1.52:3003` | yes | HTTP + SSH | repos + DB + PBS | temp PostgreSQL restore baseline passed | admin recovery documented | local credential/recovery vault | Live, upgrade/data gate | live v9 is discontinued; test staged migration to repository LTS pin before upgrade |
...
| Ollama API | LXC102 | `192.168.1.52` | 11434 | API exception | none | no web UI (Open WebUI decommissioned 2026-07-31) | TCP/API | model cache optional + PBS | LXC102 restore drill completed | no public admin UI | recovery notes local | Live protocol exception | do not expose directly through NPM |
| Immich | VM110 | `192.168.1.110` | 2283 | `foto.internal` | `http://192.168.1.110:2283` | yes | API monitor | daily DB/metadata + weekly comparison + quarterly SHA-256 + PBS | isolated DB and PBS sample restore passed | admin recovery documented | local credential/recovery vault | Live, data gate | commission and restore-test planned 2 TB removable SSD, then add later offsite copy |
...
| Homepage | LXC101 | `192.168.1.51` | 3002 | `dash.internal` | `http://192.168.1.51:3002` | yes | UI monitor | YAML config + PBS | LXC101 restore drill completed | Live | 29 cards and 4 read-only summary widgets validated |
...
| ntfy | LXC103 | `192.168.1.53` | 8093 | `alerts.internal` | `http://192.168.1.53:8093` | yes | `ops-ntfy` | config/cache + PBS | LXC103 restore drill completed | Live | deny-by-default topics and separate admin/reader/publisher identities verified |
...
| Forgejo | LXC102 | `192.168.1.52` | 3003 HTTP, 2222 SSH | `git.internal` | `http://192.168.1.52:3003` | yes | HTTP + SSH monitors | repos + DB + PBS | temp PostgreSQL restore baseline passed | Live, upgrade/data gate | live v9 is discontinued; stage and validate migration to LTS 15.0.3 |
...
| Ollama API | LXC102 | `192.168.1.52` | 11434 | none | protocol/API exception | no web UI (Open WebUI decommissioned 2026-07-31) | TCP/API monitor | model cache optional + PBS | LXC102 restore drill completed | Live protocol exception | do not expose directly through NPM |
| Immich | VM110 | `192.168.1.110` | 2283 | `foto.internal` | `http://192.168.1.110:2283` | yes | API monitor | scheduled app-aware artifacts + PBS | isolated database and PBS file-level restore passed | Live, data gate | commission and restore-test planned 2 TB removable SSD, then add offsite |
