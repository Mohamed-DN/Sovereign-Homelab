...
| Uno strumento di scrittura **girato e fallito** contava come «fatto» | «ho mandato la mail» restava senza nota anche quando `send_mail` aveva rifiutato il destinatario | `unverified_write_claim`/`unmet_write_request` guardavano *se* un tool era stato chiamato, non se aveva funzionato. Chiuso nel Guardrail (2026-07-31) leggendo il risultato di ogni tool, non solo il suo nome — vedi [momo-guardrail.md](../04_apps/momo-guardrail.md) |
| Un verbo troppo generico nella lista delle pretese accusava una frase onesta | «...persone conosciute solo se sono **registrate**» (parla della rubrica) letto come «ho registrato qualcosa» | trovato **dal vivo**, non nei 22 casi scritti a tavolino: `registrat` e `fatt` non nominano nessun vero `WRITE_TOOLS` di questa casa e producevano più falsi positivi che coperture vere. Rimossi. La guardia che accusa un'onestà è lo stesso difetto che deve prevenire |
| `pre_verify` di hermes-agent non fa quello che sembra | il piano voleva usarlo per il "rimando indietro con l'evidenza" del Guardrail su Momo | è gated su `agent._turn_file_mutation_paths`: scatta solo se il turno ha **modificato file**, mai su una chat normale. Il Guardrail di Momo dichiara la bugia invece di rimandare indietro il modello — un round di verifica in meno dell'Hermes vivo, e la differenza è scritta, non nascosta |
| La finestra di contesto di Ollama, e `options` ignorato su `/v1` | il modello non ricorda la domanda precedente, non chiama **mai** uno strumento e si presenta col nome sbagliato — tre sintomi, nessun errore | è **un guasto solo**: se il prompt supera `num_ctx`, Ollama tiene il messaggio di sistema e **scarta il resto** — cioè cronologia *e* schemi degli strumenti. Qui il prefisso era 6.694 token contro una finestra di 4.096. E `options.num_ctx` passato nel corpo in forma OpenAI **cade nel vuoto su `/v1`** (è onorato solo sul nativo `/api/chat`): il contesto si impone sul server, con `OLLAMA_CONTEXT_LENGTH`. L'allarme che avrebbe dovuto avvisare non scatta perché confronta il contesto **dichiarato** da `/api/show` (262.144) con il minimo, invece di quello **caricato**, che solo `/api/ps` mostra. Una protezione che misura la cosa sbagliata è peggio di nessuna protezione — vedi [momo-telegram.md](../04_apps/momo-telegram.md) §3-sexies |
| Modificare un file YAML con espressioni regolari | `config.yaml` troncato all'83%, e Momo senza modello: «No inference provider configured» | un YAML si modifica **come struttura** (`yaml.safe_load` → modifica dell'oggetto → `yaml.safe_dump`), mai come testo: una regex non conosce l'indentazione, che in YAML *è* la sintassi. La parte velenosa è che **un YAML tagliato a metà resta sintatticamente valido** — nessun parser protesta, e il guasto si manifesta molto dopo come una funzione mancante, lontano dalla causa. Un JSON troncato almeno dà errore subito. Fatto qui il 2026-08-01; il rottame è ancora in `config.yaml.rotto` |
