...
| 19 | **Momo non programma**, e il motore per farlo è già installato e spento | `code_execution`, `terminal`, `skills`, `delegation`, `cronjob`, `browser` e **sette ambienti sandbox** sono dentro hermes-agent e fuori dalla configurazione di Momo. Verificato il 2026-08-04. **P1 e P2 fatte** (P2 come strumento a parte, [`momo-esegui-codice.py`](../../scripts/momo/momo-esegui-codice.py), un bug vero di hermes-agent trovato e aggirato). Cercata la risposta ufficiale di NousResearch: un gateway Telegram vuole "whole-process wrapping", non solo l'isolamento del backend comandi — voce nuova [§7-bis](PIANO_MOMO_PROGRAMMATORE.md#7-bis-il-whole-process-wrapping-trovato-cercando-la-risposta-ufficiale), da pianificare a parte prima di accendere `execute_code` dentro la chat normale | [PIANO_MOMO_PROGRAMMATORE](PIANO_MOMO_PROGRAMMATORE.md) |
