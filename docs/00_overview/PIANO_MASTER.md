...
| 19 | **Momo non programma**, e il motore per farlo è già installato e spento | `code_execution`, `terminal`, `skills`, `delegation`, `cronjob`, `browser` e **sette ambienti sandbox** sono dentro hermes-agent e fuori dalla configurazione di Momo. Verificato il 2026-08-04. **P1 fatta. P2 provata dal vivo (un bug vero di hermes-agent trovato e aggirato) ma NON accesa in modo permanente**: `execute_code` condivide l'ambiente con il toolset `file` che Momo usa già oggi — accenderlo in modo permanente romperebbe `read_file`/`write_file` su percorsi reali. Decisione del proprietario richiesta — [momo-sandbox.md](../04_apps/momo-sandbox.md) §12 | [PIANO_MOMO_PROGRAMMATORE](PIANO_MOMO_PROGRAMMATORE.md) |
