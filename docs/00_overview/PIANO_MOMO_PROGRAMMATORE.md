...
| | Verdetto | Il perché |
|---|---|---|
| **Ruflo** (ex claude-flow, ruvnet) | ✅ **verificato 2026-08-08: NON installare** — [RUFLO_STUDIO_2026-08.md](RUFLO_STUDIO_2026-08.md) | letto il codice reale, non le note: **non orchestra Hermes** (zero riscontri nel codice, il tagline è marketing); **non è Node+Rust** (TypeScript quasi ovunque, Rust ridotto a 2 crate marginali); si registra come *server MCP* a cui Claude Code/Codex si collegano, non il contrario. Il catalogo dei ruoli (~99, contati) resta utile come **tassonomia/ispirazione**, non come testo da copiare — una parte ha un campo `tools:` legato al loro runtime. La cautela originale si conferma con prove: un incidente reale di marzo 2026 (script preinstall offuscato, **prompt injection deliberata** nelle descrizioni dei tool), bus factor ~1 nonostante 67k stelle |
...
| **P8** | ✅ **fatto 2026-08-08** — [RUFLO_STUDIO_2026-08.md](RUFLO_STUDIO_2026-08.md). ~99 ruoli reali (contati), i "core"/"v3" sono testo puro riusabile, i ruoli di coordinamento (github/, consensus/, hive-mind/) hanno un campo `tools:` legato al loro server MCP e vanno scartati. **Due assunzioni del piano corrette**: non è "Node+Rust" (quasi tutto TypeScript, Rust ridotto a 2 crate marginali), e non c'è nessuna integrazione con hermes-agent nel codice (il tagline "Hermes" è marketing, probabilmente il modello NousResearch via Ollama, non il framework). **La cautela si conferma con prove concrete**: un incidente reale di marzo 2026 (script preinstall offuscato, prompt injection deliberata via tool description), bus factor ~1 nonostante 67k stelle | è già il punto 19 del [PIANO_GENERALE](PIANO_GENERALE.md) | ~4 h stimate, in linea |
| **P9** | ✅ **fatto 2026-08-08** — [momo-delegation.md](../04_apps/momo-delegation.md). Nuovo strumento `delegate_to_role` sopra `delegate_task` nativo (che conosce solo leaf/orchestrator, un permesso non una persona). I 13 ruoli riusati da `scripts/hermes/roles.json` (già vivi nello sciame lineare, non reinventati). **Provato dal vivo**: una delega singola reale attraverso il vero strumento (risposta in voce da persona, non un'eco); i due freni verificati **in modo deterministico** chiamando la funzione — ciclo bloccato al 3° salto, tetto bloccato al 7° (con `MAX_HOPS=6`), nessuno dei due ha bloccato un salto legittimo. **Limite noto scritto, non nascosto**: lo scoping degli strumenti per ruolo è oggi solo una guida nel prompt, non un vincolo tecnico — `delegate_task` non ha un parametro per restringerlo | un grafo senza freni non termina | ~5 h stimate, in linea |
