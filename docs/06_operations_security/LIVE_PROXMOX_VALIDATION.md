...
- VM 110 Immich is deployed with a 500 GB data disk mounted at `/mnt/immich-library`. PBS file-level restore validation confirmed the OS disk, Immich `upload` tree, generated media directories, backups directory, and PostgreSQL data. The full boot/service drill restored VM110 to temporary VM `910`, booted it on temporary IP `192.168.1.241`, confirmed `/mnt/immich-library`, verified `immich-server`, `immich-database`, `immich-machine-learning`, and `immich-redis` healthy, confirmed API `{"res":"pong"}`, and destroyed VM `910`. A later app-aware baseline wrote `/root/sovereign-app-restore-drills/20260623T153701Z`, restored the Immich PostgreSQL dump into a temporary database with 61 public tables, and captured manifests for `/mnt/immich-library` and `/opt/sovereign-homelab`. Do not import the full photo library until offsite backup and a representative production-data restore rehearsal are complete.
