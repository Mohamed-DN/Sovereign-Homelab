...
- VM 110 Immich is deployed with a 500 GB data disk mounted at `/mnt/immich-library`. PBS boot/service recovery passed previously. On 2026-06-30, the live 31,367-file, 95.36 GB library received a new PBS checkpoint, root-only DB/metadata/SHA-256 safety bundle, verified Proxmox-vault copy, scheduled daily/weekly/quarterly protection, an isolated database restore with 61 public tables, and a 64.97 MB PBS sample-asset restore. This is strong local recovery, not 3-2-1: keep phone originals until encrypted separate-local and offsite restores also pass.
