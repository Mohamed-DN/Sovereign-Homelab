...
| Uptime Kuma | 38 live monitors after adding the 2026-06-30 CA trust portal check; full audit confirms current heartbeat state |
...
- Since 2026-06-30, all 26 private web aliases use client-side HTTPS through NPM with the shared Smallstep certificate; HTTP redirects to HTTPS. Direct upstream ports may still use HTTP inside the LAN. Install the CA root through `trust.internal` on every personal client rather than disabling certificate validation.
...
- VM 110 Immich is deployed with a 500 GB data disk mounted at `/mnt/immich-library`. PBS boot/service recovery passed previously. On 2026-07-08, Immich was upgraded to `v3.0.1` after app-aware protection, isolated database restore testing, and PBS snapshots before and after the change. The post-upgrade protection job recorded 33,306 files and 101,349,483,683 bytes, and the isolated database restore test reported 66 public tables. This is strong local recovery, not 3-2-1: keep phone originals until encrypted separate-local and offsite restores also pass.
...
- `ssd_pool` is now sparse and has healthy reported free space after clearing thick zvol reservations. Keep the live capacity gate active and still finish offsite backup before loading full Immich, Nextcloud, or Jellyfin datasets.
...
- Proxmox and service LXC host/search-domain settings were aligned to `.internal`. LXC101 and LXC102 already used `search internal`; Proxmox, LXC100, and LXC103 were corrected from legacy non-internal domain drift.
- The 2026-06-23 log review showed no failed systemd units. The only recent error-level entry was a Proxmox authentication failure from `192.168.1.100`; treat repeated occurrences as an account/session audit item before changing authentication policy.
...
| 2026-06-23 | LXC102 app-aware baseline | production LXC `102` | Vaultwarden SQLite integrity `ok`; Paperless temporary PostgreSQL restore found 72 public tables; Forgejo temporary PostgreSQL restore found 121 public tables; critical Docker volume manifests and `SHA256SUMS` written under `/root/sovereign-app-restore-drills/20260623T153506Z` |
| 2026-06-23 | VM110 Immich app-aware baseline | production VM `110` | Immich PostgreSQL dump restored into a temporary database with 61 public tables; manifests captured `/mnt/immich-library` with 32852 files and `/opt/sovereign-homelab` with 3 files; `SHA256SUMS` written under `/root/sovereign-app-restore-drills/20260623T153701Z` |
...
VM 120, VM 130, and LXC 103 are now included after their services became reachable. VM 110, VM 120, and VM 130 have full boot/service restore drills. Immich and LXC102 critical apps also have app-aware baseline evidence. Offsite protection and representative production-data restore rehearsals are still required before a full personal photo library, irreplaceable files, documents, passwords, or repositories are imported.
